...
int ECP2_BLS381_isinf(const ECP2_BLS381 *P)
{
...
/* Set P=Q */
/* SU= 16 */
void ECP2_BLS381_copy(ECP2_BLS381 *P,const ECP2_BLS381 *Q)
{
...
static void ECP2_BLS381_cmove(ECP2_BLS381 *P,const ECP2_BLS381 *Q,int d)
{
...
static void ECP2_BLS381_select(ECP2_BLS381 *P,const ECP2_BLS381 W[],sign32 b)
{
    ECP2_BLS381 MP;
    sign32 m=b>>31;
    sign32 babs=(b^m)-m;
...
/* SU= 16 */
int ECP2_BLS381_get(FP2_BLS381 *x,FP2_BLS381 *y,const ECP2_BLS381 *P)
{
    ECP2_BLS381 W;
...
/* SU= 152 */
/* Output point P */
void ECP2_BLS381_output(const ECP2_BLS381 *P)
{
    FP2_BLS381 x;
    FP2_BLS381 y;
...
/* SU= 232 */
void ECP2_BLS381_outputxyz(const ECP2_BLS381 *P)
{
    ECP2_BLS381 Q;
    if (ECP2_BLS381_isinf(P))
    {
...
void ECP2_BLS381_toOctet(octet *W,const ECP2_BLS381 *Q)
{
    BIG_384_58 b;
    FP2_BLS381 qx;
    FP2_BLS381 qy;
...
int ECP2_BLS381_fromOctet(ECP2_BLS381 *Q,const octet *W)
{
    BIG_384_58 b;
    FP2_BLS381 qx;
    FP2_BLS381 qy;
...
void ECP2_BLS381_rhs(FP2_BLS381 *rhs,const FP2_BLS381 *x)
{
...
/* SU= 232 */
int ECP2_BLS381_set(ECP2_BLS381 *P,const FP2_BLS381 *x,const FP2_BLS381 *y)
{
    FP2_BLS381 rhs;
    FP2_BLS381 y2;
...
/* SU= 232 */
int ECP2_BLS381_setx(ECP2_BLS381 *P,const FP2_BLS381 *x)
{
    FP2_BLS381 y;
    ECP2_BLS381_rhs(&y,x);
...
/* Set P+=Q */
/* SU= 400 */
int ECP2_BLS381_add(ECP2_BLS381 *P,const ECP2_BLS381 *Q)
{
    FP2_BLS381 t0;
    FP2_BLS381 t1;
    FP2_BLS381 t2;
...
/* Set P-=Q */
/* SU= 16 */
void ECP2_BLS381_sub(ECP2_BLS381 *P,const ECP2_BLS381 *Q)
{
    ECP2_BLS381 NQ;
...
/* P*=e */
/* SU= 280 */
void ECP2_BLS381_mul(ECP2_BLS381 *P,const BIG_384_58 e)
{
    /* fixed size windows */
    int i;
    int nb;
    ...
    nb=1+(BIGBITS_384_58+3)/4;

    /* convert exponent to signed 4-bit window */
    for (i=0; i<nb; i++)
    {
        w[i]=(signed char)(BIG_384_58_lastbits(t,5)-16);
        BIG_384_58_dec(t,w[i]);
        BIG_384_58_norm(t);
        BIG_384_58_fshr(t,4);
    }
    w[nb]=(signed char)BIG_384_58_lastbits(t,5);
...
void ECP2_BLS381_mul4(ECP2_BLS381 *P,const ECP2_BLS381 Q[4],const BIG_384_58 u[4])
{
    int i;
    int k;
    int nb;
    ...
    BIG_384_58 t[4];
    BIG_384_58 mt;
    sign8 w[NLEN_384_58*BASEBITS_384_58+1] = {0};
...
// Sign pivot
    ...
    for (i=0; i<nb-1; i++)
    {
        ...
        s[i]=(signed char)(2*BIG_384_58_parity(t[0])-1);
    }
...
void ECP2_BLS381_mapit(ECP2_BLS381 *Q,const octet *W)
{
    BIG_384_58 q;
    BIG_384_58 one;
    BIG_384_58 Fx;
...
#if (PAIRING_FRIENDLY_BLS381 == BLS && CURVE_SECURITY_BLS381 == 128)

static int sgn0_fp2(const FP2_BLS381 y)
{
    int sign_0;
    int sign_1;
    int zero_0;
    ...
    FP_BLS381_redc(x_0, &(y.a));
    FP_BLS381_redc(x_1, &(y.b));

    sign_0 = BIG_384_58_parity(x_0);
    sign_1 = BIG_384_58_parity(x_1);
    zero_0 = 0 - BIG_384_58_iszilch(x_0);

    return (sign_0 | (zero_0 & sign_1));
}

int ECP2_BLS381_sswu(FP2_BLS381 *x, FP2_BLS381 *y, FP2_BLS381 u)
{
    if (x == NULL || y == NULL)
        return -1;
    ...
    FP2_BLS381_from_BIGs(&a, SSWU_A2re_BLS381, SSWU_A2im_BLS381);
    FP2_BLS381_from_BIGs(&b, SSWU_B2re_BLS381, SSWU_B2im_BLS381);
    FP2_BLS381_from_BIGs(&z, SSWU_Z2re_BLS381, SSWU_Z2im_BLS381);
    FP2_BLS381_one(&gx);

    // tv1 = 1 / (Z^2 * u^4 + Z * u^2)
    FP2_BLS381_sqr(&tmp, &u);
    FP2_BLS381_mul(&tmp, &tmp, &z);
    FP2_BLS381_sqr(&tv1, &tmp);
    FP2_BLS381_add(&tv1, &tv1, &tmp);
    FP2_BLS381_inv(&tv1, &tv1);

    // x = (-B / A) * (1 + tv1)
    FP2_BLS381_add(x, &tv1, &gx);
    FP2_BLS381_mul(x, x, &b);
    FP2_BLS381_neg(x, x);
    FP2_BLS381_inv(&gx, &a);
    FP2_BLS381_mul(x, x, &gx);

    // if tv1 == 0, set x = B / (Z * A)
    if (FP2_BLS381_iszilch(&tv1)) {
        FP2_BLS381_mul(x, &b, &gx);
        FP2_BLS381_inv(&gx, &z);
        FP2_BLS381_mul(x, x, &gx);
    }
    ...
    // gx = x^3 + A * x + B
    FP2_BLS381_sqr(&gx, x);
    FP2_BLS381_add(&gx, &gx, &a);
    FP2_BLS381_mul(&gx, &gx, x);
    FP2_BLS381_add(&gx, &gx, &b);
    // y = sqrt(gx)
    FP2_BLS381_sqrt(y, &gx);
    // tv1 = y^2
    FP2_BLS381_sqr(&tv1, y);

    // If !(is_square(gx)), set x = Z * u^2 * x1 and y = sqrt(x2^3 + A * x2 + B)
    if (!FP2_BLS381_equals(&gx, &tv1)) {
        FP2_BLS381_mul(x, x, &tmp);
        FP2_BLS381_sqr(&gx, x);
        FP2_BLS381_add(&gx, &gx, &a);
        FP2_BLS381_mul(&gx, &gx, x);
        FP2_BLS381_add(&gx, &gx, &b);
...
int ECP2_BLS381_clearcofactor(ECP2_BLS381 *Q)
{
    if (Q == NULL)
        return -1;
    ...
    ECP2_BLS381 x2Q;
    FP2_BLS381 X;
    FP2_BLS381_from_BIGs(&X, Fra_BLS381, Frb_BLS381);
    ...
    ECP2_BLS381_copy(&xQ, Q);
    ECP2_BLS381_mul(&xQ, CURVE_Bnx_BLS381);
    ECP2_BLS381_copy(&x2Q, &xQ);
    ECP2_BLS381_mul(&x2Q, CURVE_Bnx_BLS381);
