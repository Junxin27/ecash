...
int ECP_BLS381_KEY_PAIR_GENERATE(csprng *RNG,octet* S,octet *W)
{
    ...
    BIG_384_58_rcopy(r,CURVE_Order_BLS381);
    if (RNG!=NULL)
    {
...
/* Validate public key */
int ECP_BLS381_PUBLIC_KEY_VALIDATE(const octet *W)
{
    BIG_384_58 q;
    BIG_384_58 r;
    BIG_384_58 k;
    ...
    if (res==0)
    {
        ...
        if (!BIG_384_58_isunity(k)) ECP_BLS381_mul(&WP,k);
...
int ECP_BLS381_SVDP_DH(const octet *S,const octet *WD,octet *Z)
{
    BIG_384_58 r;
    BIG_384_58 s;
    BIG_384_58 wx;
...
int ECP_BLS381_SP_DSA(int sha,csprng *RNG,const octet *K,const octet *S,const octet *F,octet *C,octet *D)
{
    char h[128];
    ...
    int hlen=H.len;
    if (H.len>MODBYTES_384_58) hlen=MODBYTES_384_58;
    BIG_384_58_fromBytesLen(f,H.val,hlen);
...
int ECP_BLS381_VP_DSA(int sha,const octet *W,const octet *F, octet *C,octet *D)
{
    char h[128];
    octet H= {0,sizeof(h),h};
    ...
    int hlen=H.len;
    if (hlen>MODBYTES_384_58) hlen=MODBYTES_384_58;
    ...
    if (res==0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                BIG_384_58_mod(d,r);
                if (BIG_384_58_comp(d,c)!=0) res=ECDH_INVALID;
            }
        }
    }
...
void ECP_BLS381_ECIES_ENCRYPT(int sha,const octet *P1,const octet *P2,csprng *RNG,const octet *W,const octet *M,int tlen,octet *V,octet *C,octet *T)
{

    int len;
    char z[EFS_BLS381];
    char vz[3*EFS_BLS381+1];
    char k[2*AESKEY_BLS381];
    char k1[AESKEY_BLS381];
    char k2[AESKEY_BLS381];
    char l2[8];
    char u[EFS_BLS381];
...
int ECP_BLS381_ECIES_DECRYPT(int sha,const octet *P1,const octet *P2,const octet *V,octet *C,const octet *T,const octet *U,octet *M)
{
