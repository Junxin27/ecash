...
#include "hash_to_field_BLS381.h"
...
int hash_to_field_BLS381(BIG_384_58 elems[], unsigned int nelems, unsigned int m,
    const char *msg, unsigned int msglen,
    const char *DST, unsigned int DSTlen)
{
    ...
    DBIG_384_58 dbig;
    int ret = SUCCESS;
    unsigned int L = (BIG_384_58_nbits(Modulus_BLS381) + CURVE_SECURITY_BLS381 + 7) / 8; // ceil((ceil(log2(p)) + k) / 8)
    ...
    while (byteslen) {
        BIG_384_58_dfromBytesLen(dbig, ptr_bytes, L);
        BIG_384_58_dmod(*elems, dbig, Modulus_BLS381);
        ptr_bytes += L;
        byteslen  -= L;
        elems += 1;
    }
